#!/bin/bash
# install.hk.sh - Install IOWarp Core using conda (Google Colab compatible)
...
# Modified to avoid Anaconda TOS issues by using conda-forge only
...
echo -e "${BLUE}======================================================================"
echo -e "IOWarp Core - Conda Installation (Colab Compatible)"
echo -e "======================================================================${NC}"
echo ""
...
# Configure conda channels to avoid TOS issues
echo -e "${BLUE}Configuring conda channels (removing Anaconda defaults)...${NC}"
# Remove default channels that require TOS acceptance
conda config --remove channels defaults 2>/dev/null || true
conda config --remove channels pkgs/main 2>/dev/null || true
conda config --remove channels pkgs/r 2>/dev/null || true
# Add conda-forge (no TOS required)
conda config --add channels conda-forge 2>/dev/null || true
conda config --set channel_priority strict 2>/dev/null || true
# Show configured channels
echo -e "${BLUE}Active channels:${NC}"
conda config --show channels
echo -e "${GREEN}✓ Conda channels configured (using conda-forge only)${NC}"
echo ""
