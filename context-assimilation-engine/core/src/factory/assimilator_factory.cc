...
std::unique_ptr<BaseAssimilator> AssimilatorFactory::Get(const std::string& src) {
  HLOG(kInfo, "AssimilatorFactory::Get ENTRY: src='{}'", src);

  // Check if this is a Globus web URL first
  if (src.find("https://app.globus.org") == 0) {
#ifdef CAE_ENABLE_GLOBUS
    HLOG(kInfo, "AssimilatorFactory: Detected Globus web URL, creating GlobusFileAssimilator");
    return std::make_unique<GlobusFileAssimilator>(cte_client_);
#else
    HLOG(kError, "AssimilatorFactory: Globus web URL detected but Globus support not compiled in. "
  ...
  HLOG(kInfo, "AssimilatorFactory: Extracted protocol='{}'", protocol);

  if (protocol == "file") {
    HLOG(kInfo, "AssimilatorFactory: Creating BinaryFileAssimilator for 'file' protocol");
...
#ifdef WRP_CAE_ENABLE_HDF5
    HLOG(kInfo, "AssimilatorFactory: Creating Hdf5FileAssimilator for 'hdf5' protocol");
    ...
    HLOG(kError, "AssimilatorFactory: HDF5 protocol requested but HDF5 support not compiled in. "
                  "Rebuild with -DWRP_CORE_ENABLE_HDF5=ON to enable HDF5 support.");
    return nullptr;
#endif
  } else if (protocol == "globus") {
#ifdef CAE_ENABLE_GLOBUS
    HLOG(kInfo, "AssimilatorFactory: Creating GlobusFileAssimilator for 'globus' protocol");
    // For globus protocol, return a GlobusFileAssimilator
    return std::make_unique<GlobusFileAssimilator>(cte_client_);
#else
    // Globus support not compiled in
    HLOG(kError, "AssimilatorFactory: Globus protocol requested but Globus support not compiled in. "
                  "Rebuild with -DCAE_ENABLE_GLOBUS=ON to enable Globus support.");
    return nullptr;
#endif
  }
