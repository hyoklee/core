...
// File-scope helper functions (not inside any struct or lambda) to work around
// cudafe++ crashing on thread_local variables in local struct static methods.
#if HSHM_ENABLE_CUDA
static bool chi_is_host_ptr_cuda(const void *p) {
  ...
  return a.type == cudaMemoryTypeHost || a.type == cudaMemoryTypeUnregistered;
}
static cudaStream_t chi_get_cuda_stream() {
...
static void chi_device_memcpy_cuda(void *dst, const void *src, std::size_t n) {
  if (n == 0) return;
  if (chi_is_host_ptr_cuda(dst) && chi_is_host_ptr_cuda(src)) {
    std::memcpy(dst, src, n);
    return;
  }
  cudaStream_t s = chi_get_cuda_stream();
...
static bool chi_is_host_ptr_rocm(const void *p) {
  ...
  return a.type == hipMemoryTypeHost || a.type == hipMemoryTypeUnregistered;
#else
  return a.memoryType == hipMemoryTypeHost ||
         a.memoryType == hipMemoryTypeUnregistered;
#endif
}
static hipStream_t chi_get_rocm_stream() {
...
static void chi_device_memcpy_rocm(void *dst, const void *src, std::size_t n) {
  if (n == 0) return;
  if (chi_is_host_ptr_rocm(dst) && chi_is_host_ptr_rocm(src)) {
    std::memcpy(dst, src, n);
    return;
  }
  hipStream_t s = chi_get_rocm_stream();
...
bool gpu::IpcManager::ServerInitGpuQueues(u32 queue_depth) {
  ...
  // Install device-aware memcpy + IsDevicePointer hooks.
  // Using file-scope static functions (defined above) to avoid cudafe++
  // crashing on thread_local or complex templates inside lambdas.
#if HSHM_ENABLE_CUDA
  chi::g_device_aware_memcpy.store(chi_device_memcpy_cuda,
                                   std::memory_order_release);
#elif HSHM_ENABLE_ROCM
  chi::g_device_aware_memcpy.store(chi_device_memcpy_rocm,
                                   std::memory_order_release);
#else
